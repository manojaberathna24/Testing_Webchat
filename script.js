...
5. Always mention stock status when discussing products`;
    ...
    async loadConfig() {
                ...
                if (!error && data) {
                    for (const item of data) {
                        ...
                        if (item.key === 'chatbot_model') {
                            // Validate model - fall back to default if outdated/deprecated
                            this.model = VALID_MODELS.includes(item.value) ? item.value : DEFAULT_MODEL;
                        }
                    }
                    if (this.apiKey) {
                        console.log('Loaded API config from Supabase, model:', this.model);
                        return;
                    }
                }
            } catch (e) {
        ...
        const storedModel = localStorage.getItem('chatbot_model');
        // Validate stored model - use default if it's a deprecated/removed model
        this.model = (storedModel && VALID_MODELS.includes(storedModel)) ? storedModel : DEFAULT_MODEL;
